...
if( isset($_POST["data"]) && !empty($_POST["data"]) ){
    ...
    if( $response["error"] == 0 ){
        ...
        <script>
        window.onload = function() {
            alert("<?php echo direction("Erorr while loading payment data from payment gateway.","حدث خطأ اثناء تحميل بيانات الدفع.") ?>");
	...
	<script>
	window.onload = function() {
		alert("<?php echo direction("Erorr while loading post data payment data.","حدث خطأ اثناء تحميل بيانات الدفع.") ?>");
		window.location.href = "?v=Home";
	};
	</script>
	<?php
