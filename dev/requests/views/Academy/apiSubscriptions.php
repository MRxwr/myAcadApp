...
if( !isset($_GET["userId"]) || empty($_GET["userId"]) ){
	...
	if( $orders = selectDB2("`id`, `date`, `academyId`, `gatewayId`, `isTournament`, `tournamentId`, `teamName`, `teamDetails`, `eventId`, `status`","orders","`userId` = '{$_GET["userId"]}' AND `status` != '0' ORDER BY `id` DESC LIMIT {$limit} OFFSET {$offset}") ){
        for( $i = 0; $i < sizeof($orders); $i++ ){
