...
if( !isset($_POST["orderId"]) || empty($_POST["orderId"]) ){
	...
	if( $order = selectDB2("`id`, `date`, `isTournament`, `paymentMethod`, `enAcademy`, `arAcademy`, `enSession`, `arSession`, `enSubscription`, `arSubscription`, `subscriptionQuantity`, `jersyQuantity`, `totalSubscriptionPrice`, `totalJersyPrice`, `voucher`, `total`, `tournamentId`, `teamDetails`, `eventId`, `eventDetails`","orders","`id` = '{$_POST["orderId"]}'") ){
        if( $order[0]["isTournament"] == 1 ){
            ...
            $data[0]["id"] = $order[0]["id"];
            $data[0]["date"] = $order[0]["date"];
            $data[0]["enTitle"] = isset($eventDetails["enEvent"]) ? $eventDetails["enEvent"] : "";
            $data[0]["arTitle"] = isset($eventDetails["arEvent"]) ? $eventDetails["arEvent"] : "";
            $data[0]["imageurl"] = isset($event[0]["imageurl"]) ? $event[0]["imageurl"] : "";
            $data[0]["header"] = isset($event[0]["header"]) ? $event[0]["header"] : "";
            $data[0]["location"] = isset($event[0]["location"]) ? $event[0]["location"] : "";
            $data[0]["locationImage"] = isset($event[0]["locationImage"]) ? $event[0]["locationImage"] : "";
            $data[0]["gateDate"] = isset($event[0]["gameDate"]) ? $event[0]["gameDate"] : "";
            $data[0]["gateTime"] = isset($event[0]["gameTime"]) ? $event[0]["gameTime"] : "";
            $data[0]["price"] = isset($eventDetails["price"]) ? $eventDetails["price"] : 0;
            $data[0]["total"] = isset($eventDetails["total"]) ? $eventDetails["total"] : 0;
            $data[0]["voucher"] = $order[0]["voucher"];
            $data[0]["fields"] = isset($eventDetails["fields"]) ? json_decode($eventDetails["fields"], true) : [];
            $data[0]["arena"] = ( !empty($event[0]) && (!empty($event[0]["isIndoor"]) && $event[0]["isIndoor"] != 0) ) ? popupMsg($requestLang,"Indoor","داخلي") : popupMsg($requestLang,"Outdoor","خارجي");
            $response = $data;
        }else{
