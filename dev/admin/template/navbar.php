<nav class="navbar navbar-inverse navbar-fixed-top" style="background-color: #000d2c !important;">
			<div class="mobile-only-brand pull-left" style="background-color: #000d2c !important;">
				<div class="nav-header pull-left">
					<div class="logo-wrap">
						<a href="index.php" style="white-space: nowrap;">
							<img class="brand-img" src="../logos/logoNew.png" alt="brand" style="width:25px;height:25px"/>
			...
			<div id="mobile_only_nav" class="mobile-only-nav pull-right" style="background: #000d2c;">
				<ul class="nav navbar-right top-nav pull-right">
					<li class="dropdown auth-drp">
						<a href="#" class="dropdown-toggle pr-0" data-toggle="dropdown"><img src="../logos/logoNew.png" alt="user_auth" class="user-auth-img img-circle"/><span class="user-online-status"></span></a>
					...
					<li class="dropdown alert-drp">
						<a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
							...
							<?php 
								if( $listOfOrdrs = selectDB("orders","`status` LIKE '0' AND `status` LIKE '1' ORDER BY `date` DESC") ){
								    $numberOfRows = sizeof($listOfOrdrs);
								}else{
								    $numberOfRows = 0;
								}
							?>
							...
							<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 229px;">
								<div class="streamline message-nicescroll-bar" style="overflow: hidden; width: auto; height: 229px;">
									...
									if( !empty($numberOfRows) ){
    									for( $i = 0; $i < sizeof($listOfOrdrs); $i++ )
    									{
    									?>
    									<div class="sl-item">
    										<a href="product-orders.php?info=view&id=<?php echo $listOfOrdrs[$i]["orderId"] ?>">
    											<div class="icon bg-blue">
    												<i class="zmdi zmdi-info"></i>
    											</div>
    											<div class="sl-content">
    												<span class="inline-block capitalize-font  pull-left truncate head-notifications txt-danger">Order Id:<?php echo $listOfOrdrs[$i]["orderId"] ?>
    												</span>
    												<span class="inline-block font-11  pull-right notifications-time"><?php echo $listOfOrdrs[$i]["date"] ?></span>
