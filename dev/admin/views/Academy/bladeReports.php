...
<div class="row">
	...
	<select class="form-control" name="academyId">
		...
		for( $z = 0; $z < $count; $z++ ){
			...
			if( $academies = selectDB("academies","`status` != '4' {$id} ORDER BY `{$orderBy}` ASC") ){
	...
	<select class="form-control" name="voucher">
		...
		<?php
			...
			if( $userType != 0 && $userType != 8 && !empty($academiesList) ){
				$academyConditions = array();
				foreach( $academiesList as $academyId ){
					$academyConditions[] = "`academyIds` LIKE '%\"{$academyId}\"%'";
				}
				if( !empty($academyConditions) ){
					$voucherQuery .= " AND (" . implode(" OR ", $academyConditions) . ")";
				}
			}
...
if ( isset($_POST["endDate"]) ){
	...
	if ( !empty($_POST["academyId"]) ){
		$where .= " AND `academyId` = '{$_POST["academyId"]}'";
	}
