		...
		if( $sessions = selectDB("sessions","`status` = '0' AND `academyId` LIKE '{$_GET["code"]}' ORDER BY `{$orderBy}` ASC") ){
			for( $i = 0; $i < sizeof($sessions); $i++ ){
				...
				<td class="text-nowrap">
					<a href="<?php echo "?v=Sessions_Subscriptions&code={$_GET["code"]}&sessionId={$sessions[$i]["id"]}" ?>" class="subscribe btn btn-success" data-toggle="tooltip" data-original-title=""><?php echo direction("Subscriptions","الإشتراكات") ?></a>
					<a id="<?php echo $sessions[$i]["id"] ?>" class="edit btn btn-warning" data-toggle="tooltip" data-original-title="<?php echo direction("Edit","تعديل") ?>"> <i class="fa fa-pencil text-inverse m-r-10"></i></a>
