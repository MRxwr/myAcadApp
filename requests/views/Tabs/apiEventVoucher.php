...
$dateApproved = false;
if( isset($_POST["code"]) && !empty($_POST["code"]) && $voucher = selectDB("vouchers","`code` = '{$_POST["code"]}' AND `typeOfVoucher` = '2' AND `hidden` = '0' AND `status` = '0'")){
    if( !isset($_POST["eventId"]) || empty($_POST["eventId"]) ){
        $response = array(
            "msg" => 'event is required.',
            "msgAr" => 'يجب إدخال الحدث',
        );
    ...
    if( !empty($voucher[0]["eventIds"]) ){
        ...
        if(  in_array($_POST["eventId"],$voucher[0]["eventIds"]) ){
            $eventApproved = true;
        }else{
            $eventApproved = false;
            $response = array(
                "msg" => 'voucher is not valid for this event.',
                "msgAr" => 'لا يمكن تطبيق هذا الكود على هذا الحدث',
            );
            echo outputError($response);die();
        }
