...
<div class="pull-left">
	<h6 class="panel-title txt-dark"><?php echo direction("Field Voucher Details","تفاصيل كود الخصم للملعب") ?></h6>
</div>
...
<div class="panel-body">
	<form class="" method="POST" action="?v=<?php echo $_GET["v"] ?>" enctype="multipart/form-data">
		<div class="row m-0">
			...
			<label class="control-label mb-10"><?php echo direction("Field","الملعب") ?></label>
			<select class="form-control" name="fieldIds[]" class="form-control" id="mySelect" multiple style="height: 200px">
				<?php
				...
				for( $z = 0; $z < $count; $z++ ){
					$id = ( isset($fieldsList[$z]) && !empty($fieldsList[$z]) ) ? "AND `id` = '{$fieldsList[$z]}'" : "AND `id` != '0'";
					if( $fields = selectDB("fields_list","`status` = '0' {$id} ORDER BY `{$orderBy}` ASC") ){
						for( $i = 0; $i < sizeof($fields); $i++ ){
							$area = selectDB("countries","`id` = '{$fields[$i]["area"]}'");
							$areaTitle = direction($area[0]["areaEnTitle"],$area[0]["areaArTitle"]);
							$fieldTitle = direction($fields[$i]["enTitle"],$fields[$i]["arTitle"]);
							echo "<option value='{$fields[$i]["id"]}'>{$fieldTitle} - {$areaTitle} </option>";
						}
					}
				}
				?>
...
<div class="panel panel-default card-view">
<div class="panel-heading">
	<div class="pull-left"><h6 class="panel-title txt-dark"><?php echo direction("List of Field Vouchers","قائمة كوبونات الملاعب") ?></h6></div>
	<div class="clearfix"></div>
</div>
<div class="panel-wrapper collapse in">
<div class="panel-body">
...
<div class="table-responsive">
	<table class="table display responsive product-overview mb-30" id="myTable">
		...
		$count = (is_array($fieldsList) && !empty($fieldsList)) ? count($fieldsList) : 1;
		for( $z = 0; $z < $count; $z++ ){
			$id = ( isset($fieldsList[$z]) && !empty($fieldsList[$z]) ) ? "AND `fieldIds` LIKE '%{$fieldsList[$z]}%'" : "AND `id` != '0'";
			if( $vouchers = selectDB("vouchers","`status` = '0' AND `hidden` != '2' AND `typeOfVoucher` = '0' {$id}") ){
				for( $i = 0; $i < sizeof($vouchers); $i++ ){
					...
					$cleanedFieldId = str_replace(['[', ']', '"'], '', $vouchers[$i]["fieldIds"]);
					$vouchers[$i]["fieldIds"] = explode(',', $cleanedFieldId);
					for( $j = 0; $j < sizeof($vouchers[$i]["fieldIds"]); $j++ ){
						if( $fieldData = selectDB("fields_list","`id` = '{$vouchers[$i]["fieldIds"][$j]}'") ){
							$field .= direction($fieldData[0]["enTitle"],$fieldData[0]["arTitle"]) . " - ";
						}else{
							$field .= "";
						}
					}
					...
					<div style="display:none">
						...
						<label id="field<?php echo $vouchers[$i]["id"]?>"><?php echo json_encode($vouchers[$i]["fieldIds"]) ?></label>			
					</div>				
					</td>
					</tr>
					<?php
...
<script>
	...
	$(document).on("click",".edit", function(){
		...
		// field is json objext with list of ids 
		var field = JSON.parse(field);
		$("#mySelect").val(field).trigger('change');
