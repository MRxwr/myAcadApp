...
<div class="table-responsive">
	<table class="table display responsive product-overview mb-30" id="myTable">
        ...
        // add where to the quesy to check if the data passed 30 days or not
			if( $academies = selectJoinDB("orders",$joinData,"t.date <= DATE_SUB(NOW(), INTERVAL 30 DAY) AND (t.status = '1' OR t.status = '4') AND t.tournamentId = '0' GROUP BY t.academyId ORDER BY t.id DESC") ){
                $gendersList = array(
                    "1" => direction("Man","رجل"),
