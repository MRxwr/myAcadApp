			...
			for( $i = 0; $i < sizeof($purchases); $i++ ){
				...
				<td class="text-nowrap">
                    ...
                    if( $purchases[$i]["status"] == 0 ){
                        ?>
                            <a class="btn btn-danger btn-outline btn-icon left-icon" onclick="confirm('Are you sure you want to cancel this purchase?');" href="?v=<?php echo "{$_GET["v"]}&cancel={$purchases[$i]["id"]}" ?>"><?php echo direction("Cancel","الغاء") ?></a>
                        <?php
                    }
                    ?>
				</td>
