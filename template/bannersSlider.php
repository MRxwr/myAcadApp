		...
		if( $banners = selectDB("banners","`status` = '0' AND `hidden` = '0' ORDER BY `order` ASC") ){
			for( $i = 0; $i < sizeof($banners) ; $i++ ){
				...
				if( $banners[$i]["type"] == 0 ){
					...
					echo "<div class='item'><div class='play_video' style='background-image: url(logos/{$banners[$i]["imageurl"]});background-size: fill;'><a href='{$banners[$i]["link"]}' class='watch_btn'><i class='fal fa-play-circle'></i></a></div>";
				}
			}
		}
		?>
